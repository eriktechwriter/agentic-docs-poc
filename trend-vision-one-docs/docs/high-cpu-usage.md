...
3.  Apply the best practices on [Performance tips for anti-malware](performance-tips-for-anti-malware.md) and [Performance tips for intrusion prevention](performance-tips-prev.md).
...
5.  If a [recommendation scan](recommendation-scans-intro.md) is being performed, try running scans during a time when the computer is less busy, or (if the computer is a VM) allocating more vCPUs.
...
7.  If high CPU usage still continues, try temporarily stopping the agent. Verify that the issue stops when the agent is stopped. If it does, [collect diagnostic information](create-diagnostic-package-logs.md) and give it to your support provider.
