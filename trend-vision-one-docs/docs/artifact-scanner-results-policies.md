...
You can integrate Trend Micro Artifact Scanner (TMAS) results into Container Security admission control policies. For information on how to install and set up the CLI, see [Integrate Trend Micro Artifact Scanner (TMAS) into a CI/CD pipeline](integrating-tmas-ci-cd-pipeline.md).
...
Next, create a [Container Protection policy](policies.md) that utilizes the artifact scanner's results.
