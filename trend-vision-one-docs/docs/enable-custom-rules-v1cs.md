...
5.  Complete configuration of the policy through policy as code.
    ...
    Custom rulesets are a part of the policy. To ensure there is no deviation when using a shared policy, the custom rules features requires users to use policy as code with a cluster-managed policy. Learn more about [Cluster-managed policies](cluster-managed-policies.md).
