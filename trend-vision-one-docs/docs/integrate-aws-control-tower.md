...
1.  In the Server & Workload Protection console, go to **Administration → User Management → API Keys** and click *New*. Select a name for the key and the *Full Access* role. Be sure to save the key as it cannot be retrieved later. This key will be used to authenticate the automation from the AWS Control Tower Master to the console API. For more information, see [Create an API key](send-your-first-request-using-api.md).
...
5.  Once all your accounts have been imported, automate [agent installation](install-agent.md) and activate protection.
...
To remove the lifecycle hook, identify and delete the CloudFormation stack. Protection for Managed Accounts that have already been added will remain in place. Removing an AWS account from the Trend Vision One console is now handled by the Cloud Accounts app. For details on removing an AWS account from Cloud Accounts, see [AWS accounts](aws-accounts.md).
