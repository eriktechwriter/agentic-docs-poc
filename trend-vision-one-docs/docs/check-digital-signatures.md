...
You can also validate the software's checksums and digital signatures for the security updates and agent modules. See [How Server & Workload Protection validates update integrity](upgrades.md).
...
- Export a .zip file following the instructions in [Export the agent installer](get-agent-software.md).
...
The procedures below describe how to manually check a digital signature on an installer file. To automate this check, include it in your [agent deployment scripts](deployment-scripts-add-computers.md).
...
Instead of checking the signature on the .rpm file manually, consider [using a deployment script](deployment-scripts-add-computers.md). Otherwise, continue with the below steps.
...
Instead of checking the signature on the .deb file manually, consider [using a deployment script](deployment-scripts-add-computers.md).
