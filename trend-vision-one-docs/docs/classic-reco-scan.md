...
:::danger[Warning]
Do not enable [Auto apply core Endpoint & Workload rules](config-intrusion-prevention-rules.md) when using classic recommendation scan.
:::

During a classic [recommendation scan](recommendation-scans-intro.md), agents scan the following:
...
- [Create a scheduled task](schedule-reco-scan.md) that runs recommendation scans according to a schedule that you configure. You can assign the scheduled task to all computers, one individual computer, a defined computer group, or all computers protected by a particular policy.
...
- Use the Server & Workload Protection command-line interface (CLI) to initiate a classic recommendation scan. See [Command-line utilities](command-line-basics.md).

- Use the Server & Workload Protection application programming interface (API) to initiate a classic recommendation scan. See [How to use the Server & Workload Protection REST API](api-sdk-devops-tools-for-automation.md).
...
- For communication issues, *protocol error* often appears in the body of the error message. To resolve this issue, ensure that you are using agent-initiated communication. See [Activate and protect agents using agent-initiated activation and communication](agent-initiated-activation.md).

- If you receive a *Recommendation Scan Failure* message on your server, [create a diagnostic package from the agent](create-diagnostic-package-logs.md) and [contact support](https://cloudone.trendmicro.com/support).
