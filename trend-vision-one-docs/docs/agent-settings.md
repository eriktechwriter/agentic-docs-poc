...
:::tip
You can automate agent-related system setting changes using the Server & Workload Protection API. For examples, see [Configure Policy, Computer, and System Settings](configure-system-settings.md).
:::

### Agent-initiated activation (AIA) {#agent-initiated-activation-aia}

In addition to activating new agents in Server & Workload Protection (such as via a cloud connector or manually adding a new computer on **Computers**), but you can also (or instead) allow agents to automatically activate themselves. See also [Activate and protect agents using agent-initiated activation and communication](agent-initiated-activation.md).
...
- **Allow Trend Vision One Virtual Desktop Infrastructure (VDI) support and cloned virtual machines**: Enabling VDI support locks the following system settings.
  ...
  2.  Disable **Automatically upgrade agents on activation** for the corresponding operating system. See [Automatically upgrade agents on activation](autoupgrade-agents-activation.md).
...
**Automatically upgrade agents on activation** During activation, upgrade the agent to the latest software version that's compatible with Server & Workload Protection. Linux computers only. See also [Automatically upgrade agents on activation](autoupgrade-agents-activation.md).
