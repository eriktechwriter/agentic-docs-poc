...
3.  Set up an Internet Access On-Premises Gateway by clicking **Go to Service Gateway Inventory**.
    ...
    1.  Select an existing Service Gateway that identifies your corporate location, or [deploy a new Service Gateway virtual appliance](deployment-guides.md) for the Zero Trust Internet Access On-Premises Gateway service.

        :::warning[Important]
        Disable **Cloud Service Extension** on the Service Gateway when using the Internet Access On-Premises Gateway service. The cloud service extension might interfere with normal operations of the on-premises gateway. For more information, see [Configure Service Gateway settings](config-service-gateway-settings.md).
        :::

    2.  Install and enable the Zero Trust Internet Access On-Premises Gateway service. For details, see [Manage services in Service Gateway](managing-services-service-gateway.md).
...
8.  Configure and apply PAC files to forward HTTP/HTTPS traffic to the on-premises gateway.
    ...
    1.  Add the FQDN or IP address of the on-premises gateway [to one or multiple PAC files](pac-file-configuration.md) that you use for proxy settings.

    2.  [Apply the PAC files](pac-file-deployment.md) to deployed Secure Access Modules.

9.  [Configure bandwidth control](configuring-bandwidth-control.md) to optimize network performance on the on-premises gateway.

**Related information**

- [Configure upstream proxy rules](configuring-upstream-proxy-rules.md "Set up rules for user web activity traffic using upstream proxies on your Internet Access On-Premises Gateway.")
- [Configure bandwidth control](configuring-bandwidth-control.md "Reduce network congestion on your on-premises gateway by configuring bandwidth control.")
- [Configure reverse proxy mode](configuring-reverse-proxy-mode.md "Configure reverse proxy mode on an Internet Access On-Premises Gateway in order to secure the use of your private general or generative AI applications.")
- [Configure AI proxy for multiple LLMs](private-ai-service-llm-integration.md "AI proxy is a reverse proxy that allows you to use multiple Large Language Model (LLM) services with different API protocols.")
- [Enable FTP proxy on an Internet Access On-premises gateway](ftp-proxy-internet-premises-gateway.md)
