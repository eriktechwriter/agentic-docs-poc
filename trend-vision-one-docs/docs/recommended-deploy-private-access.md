...
First, you [create a connector group and deploy the connector](private-access-connector-deployment.md). The connector group serves to expose the VLANs, allowing external users to access the internal services hosted within them.
...
After deploying the connector, it is essential to [add the internal applications](add-internal-application-pac.md) in the Trend Vision One console to enable the connector to identify which applications it can access on the internal network.
...
To manage access permissions for the applications we just created, it is essential to [create private access rules](creating-pac-rule.md).
