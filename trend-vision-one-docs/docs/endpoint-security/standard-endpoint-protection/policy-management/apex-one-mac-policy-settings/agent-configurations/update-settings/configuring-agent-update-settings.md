...
For a detailed explanation of agent updates, see [Update Settings](.).
...
1.  Select **Agents download updates from the Trend Micro ActiveUpdate server when unable to connect to the Apex One (Mac) server** to allow agents to download updates from the Trend Micro ActiveUpdate server.
    ...
    :::note
    If a Trend Vision One Endpoint Security agent only has an IPv6 address, read the IPv6 limitations for agent updates in [Pure IPv6 Agent Limitations](./pure-ipv6-agent-limitations).
    :::
