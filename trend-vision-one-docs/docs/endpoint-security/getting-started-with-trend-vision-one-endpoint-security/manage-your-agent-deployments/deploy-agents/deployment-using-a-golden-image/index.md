...
Endpoint Security offers separate settings for physical/persistent desktops and non-persistent desktops. Virtual desktops which are not turned on regularly are automatically removed based on the agent settings. The time interval is set separately for persistent and non-persistent desktops. For information regarding inactive agent removal, see [Endpoint Settings](../../../../endpoint-inventory/global-settings/sensor-settings).
...
- [Creating a golden image with the agent software](./create-golden-agent-image "Prepare a template to easily deploy endpoint agents and sensors to cloned desktops on physical machines, as well as persistent and non-persistent virtual desktops.")
- [Updating the agent for golden image templates](./updating-agent-for-templates "Update the agent program on virtual desktops created using a golden image.")
