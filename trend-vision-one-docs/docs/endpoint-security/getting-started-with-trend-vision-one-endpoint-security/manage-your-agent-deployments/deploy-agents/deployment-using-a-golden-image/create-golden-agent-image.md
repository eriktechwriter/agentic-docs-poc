...
- Updating the agent program requires creating a snapshot image before running the Image Setup Tool. For more information about updating a golden image, see [Updating the agent for golden image templates](./updating-agent-for-templates).
:::
...
3.  If you are deploying a Server & Workload Protection agent, configure **Agent-Initiated Activation**.
    ...
    3.  Select **Allow Trend Vision One Virtual Desktop Infrastructure (VDI) support and cloned virtual machines**.
        ...
        :::warning[Important]
        Enabling this feature locks other Agent-Initiated Activation settings. For more information, see [Agent settings](../../../../server-workload-protection/administration/manage-agents-protected-computers/agent-settings).
        :::
...
5.  Download the Agent Installer package you want to use and install the agent on the source desktop.
    ...
    - [Deploy agents with Standard Endpoint Protection features](../sep-agent-deployment)

    - [Deploy agents with Server & Workload Protection features](../swp-agent-deployment)

    - [Deploy agents with only Endpoint Sensor features](../endpoint-sensor-agent-deployment)
...
8.  If you want to facilitate future agent upgrades, create a snapshot of your source desktop.
    ...
    For more information, see [Updating the agent for golden image templates](./updating-agent-for-templates).
