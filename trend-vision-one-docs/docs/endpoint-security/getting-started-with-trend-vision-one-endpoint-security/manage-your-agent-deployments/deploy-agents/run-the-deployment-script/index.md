...
7.  Select the **Proxy for deployment**.
    ...
    - **Service Gateway**: The agent installer attempts to connect using a deployed Service Gateway with Forward Proxy Service enabled.
      ...
      This option requires a Service Gateway with Forward Proxy Service installed and enabled. For more information, see [Deploy a Service Gateway and Configure Firewall Exceptions](../../../deploy-a-service-gateway-and-configure-firewall-exceptions).

    :::warning[Important]
    The deployment script does not utilize the custom proxies defined in the [Agent Installer Proxy settings](../../../../endpoint-inventory/global-settings/agent-installer-proxy-settings). Agents adopt the [Runtime Proxy settings](../../../../endpoint-inventory/global-settings/runtime-proxy-settings) assigned to their endpoint group after installation and registration is successfully completed.
    :::
...
- [Troubleshooting the Deployment Script](./troubleshooting-deployment-script "Review solutions to resolving typical issues that may arise when attempting to use the deployment script to deploy endpoint agents.")
