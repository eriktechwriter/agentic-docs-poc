...
Service Gateway appliances provide multiple features to connect data from on-premises Trend Micro solutions and various third-party tools. Install a Service Gateway and enable the **Forward proxy** feature to minimize the overhead associated with configuring firewall exceptions. You can also set up backup proxy sources in your [Endpoint Inventory Global Settings](../../endpoint-inventory/global-settings) in the event that your Service Gateway connection is unavailable.
...
2.  For customers that do not have an existing Service Gateway deployed, install a Service Gateway.
    ...
    5.  Install the Service Gateway virtual appliance.
        ...
        - [Deploying a Service Gateway Virtual Appliance with VMware ESXi](./deploy-service-gateway-vmware-esxi)

        - [Deploying a Service Gateway Virtual Appliance with Microsoft Hyper-V](./deploy-service-gateway-hyper-v)

        For a complete list of Service Gateway system requirements, see [Service Gateway appliance system requirements](./service-gateway-sys-req).
...
6.  Configure your firewall exceptions for Trend Vision One Endpoint Security.
    ...
    :::note
    If you are enabling or using other Trend Vision One features, you may need to configure additional firewall exceptions. Refer to the [complete firewall exceptions lists](../../../getting-started/getting-started-with-trend-vision-one/firewall-exception-requirements-for-trend-vision-one) to find out about other feature requirements.
    :::
...
- [Service Gateway appliance system requirements](./service-gateway-sys-req "View detailed Service Gateway system requirements based on service deployment.")
- [Service Gateway sizing guide for endpoints](./gateway-endpoint-sizing-guide "Select a Service Gateway appliance specification based on type and number of endpoints.")
- [Deploying a Service Gateway Virtual Appliance with VMware ESXi](./deploy-service-gateway-vmware-esxi "Discover how to install the Service Gateway virtual appliance on VMware ESXi.")
- [Deploying a Service Gateway Virtual Appliance with Microsoft Hyper-V](./deploy-service-gateway-hyper-v "Instructions on how to use the Command Line Interface (CLI) to deploy a Service Gateway virtual appliance with Microsoft Hyper-V.")
