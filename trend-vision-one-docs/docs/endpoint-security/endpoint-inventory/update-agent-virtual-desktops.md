...
:::warning[Important]
To update your golden image, you must have a snapshot of the source desktop created before running the Image Setup Tool and exporting the golden image. If you do not have a snapshot available, follow the steps in [Creating a golden image with the agent software](../getting-started-with-trend-vision-one-endpoint-security/manage-your-agent-deployments/deploy-agents/deployment-using-a-golden-image/create-golden-agent-image) to create a new snapshot and golden image.
