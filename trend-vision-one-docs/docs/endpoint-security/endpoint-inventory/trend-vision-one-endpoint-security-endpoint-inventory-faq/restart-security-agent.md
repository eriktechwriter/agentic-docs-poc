...
:::note
This FAQ is about how to restart the Trend Vision One Endpoint Security agent after using the agent interface to shut down the agent on a Windows endpoint. For information about shutting down the agent, see [Agent Interface](../../endpoint-security-configuration/endpoint-security-policies-unified-assignment/policies/policies-agent-interface).

For steps on shutting down agent services manually, see [How to manually terminate Trend Vision One Endpoint Security agent processes](./manually-stop-agent-processes).
