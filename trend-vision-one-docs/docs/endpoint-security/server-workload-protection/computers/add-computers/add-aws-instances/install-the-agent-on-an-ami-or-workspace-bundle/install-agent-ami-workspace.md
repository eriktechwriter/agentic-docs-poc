...
AWS Accounts in Trend Vision One are now managed by the Cloud Accounts app. To add new AWS accounts, see [Connect an AWS account using CloudFormation](../../../../../../cloud-security/cloud-accounts/aws-accounts/connect-and-update-aws-accounts/adding-aws-account).
...
For information on how to protect existing Amazon EC2 instances and Amazon WorkSpaces withServer & Workload Protection, see [Install the agent on Amazon EC2 and WorkSpace instances](../install-the-agent-on-amazon-ec2-and-workspaces/install-agent-amazon-ec2-workspaces).

For information on how to protect Amazon WorkSpaces after already protecting your Amazon EC2 instances, see [Protect Amazon WorkSpaces if you already added your AWS account](../add-amazon-workspaces/add-amazon-workspaces).
...
You need to indicate whether or not to allow agent-initiated activation. For instructions, see [Install the agent on Amazon EC2 and WorkSpaces](../install-the-agent-on-amazon-ec2-and-workspaces/install-agent-amazon-ec2-workspaces) \> [Configure the activation type](../install-the-agent-on-amazon-ec2-and-workspaces/install-agent-amazon-ec2-workspaces).
...
1.  In the Server & Workload Protection console, create an event-based task with these parameters:
    ...
    For details on creating event-based tasks, see [Automatically assign policies based on AWS EC2 instance tags](../../../../administration/automate/automate-using-the-console/auto-assign-provider-tags-labels).
...
3.  On the master EC2 instance or WorkSpace, reactivate the agent by re-running the activation command on the agent, or by clicking the **Reactivate** button in the Server & Workload Protection console. For details, see [Activate the agent.](../../../../administration/manage-agents-protected-computers/activate-the-agent/activate-agent) The re-activation causes the event-based task to assign the policy to the master. The master is now protected.
...
- To create an AMI on Linux or Windows, see [Create an Amazon EBS-backed AMI](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/creating-an-ami-ebs.md).
