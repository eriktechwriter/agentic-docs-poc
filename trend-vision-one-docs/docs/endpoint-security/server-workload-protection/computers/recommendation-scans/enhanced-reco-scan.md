...
:::danger[Warning]
Do not enable [Auto apply core Endpoint & Workload rules](../../configure-protection-modules/configure-intrusion-prevention/set-up-intrusion-prevention/set-up-intrusion-prevention) when using enhanced recommendation scan.
:::

The enhanced [recommendation scan](.) improves upon the classic recommendation scan in the following ways:

- Automatically runs within an hour after activation and at least once every 24 hours. This scan is short and efficient to avoid disrupting operations.
...
- Fewer limitations than the classic recommendation scan:
  ...
  - Can recommend new web application rules, if applicable.
...
Enhanced recommendation scans configured to automatically implement recommendations do so immediately for the first scan, as soon as new recommendations are received. These scans do not wait for an ongoing or scheduled scan to be run.

The enhanced recommendation scan has the following requirements:

- Agent version 20.0.1-21510 or later (see supported features by version for [Linux](../../administration/manage-agents-protected-computers/configure-agent-version-control/agent-platform-compatibility/supported-features-linux-platform), [Windows](../../administration/manage-agents-protected-computers/configure-agent-version-control/agent-platform-compatibility/supported-features-windows-version), or [Windows Server](../../administration/manage-agents-protected-computers/configure-agent-version-control/agent-platform-compatibility/support-features-winserver))
...
- Internet of Things (IoT) traffic to Amazon Web Services (AWS) permitted through firewalls
  ...
  If the agent does not receive any recommendations for 36 hours and either IoT traffic to AWS is not permitted through the firewalls or the enhanced recommendation scan fails, the agent automatically falls back to the [classic recommendation scan](./classic-reco-scan). Upon receiving recommendations from the enhanced recommendation scan, the agent immediately resumes using the enhanced recommendation scan.

Agents that do not meet the requirements for the enhanced recommendation scan automatically use the [classic recommendation scan](./classic-reco-scan) instead.
...
1.  Enable and configure [automatic rule implementation](./auto-imp-recommendations).

2.  Choose one of the following:

    - [Schedule a recommendation scan](./schedule-reco-scan).

    - [Configure an ongoing recommendation scan](./config-ongoing-reco-scan).
...
This scan for recommendations is similar to the [classic recommendation scan](./classic-reco-scan) but with a timeout for receiving results within 10 minutes. Clicking **Scan for Recommendations** disables the button during this timeout. If the recommendation scan results take longer than 10 minutes, the button becomes available so you can try again.
