...
A relay is an agent that is configured to redistribute [software and component updates](../upgrade-server-workload-protection/upgrades) to other agents. Relays help your deployment perform well as it grows and scales.

Default relays are available inside Server & Workload Protection. Agents should be able to use them if they can [connect to Server & Workload Protection](../../troubleshooting/swp-port-numbers). You might need more relays for performance or cost reasons.

Alternatively, software updates (but not component updates) can be [distributed by a local mirror web server](../upgrade-server-workload-protection/use-a-web-server-to-distribute-software-updates/web-server-distribute).
...
- New relays for Deep Security Agent version 20.0.0-3771 or earlier cannot connect to [Workload Security relays via proxy](../configure-proxies/configure-proxies-server-workload). This support was added in the agent version 20.0.0-3964.
...
For instructions on how to deploy your own relays, see [Deploy more relays](./deploy-more-relays/deploy-more-relays).
