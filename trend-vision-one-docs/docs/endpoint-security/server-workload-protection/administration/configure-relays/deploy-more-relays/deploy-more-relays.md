...
[If you need to deploy more relays](../how-relays-work), you must:
...
- *Geographic region and distance*: If you're deploying your own relays, each geographic region should have its own [relay group](../how-relays-work) with at least 2 relays and agents should use relays in their same geographic region. Long distance and network latency can slow down update redistribution. Downloading from other geographic regions can also increase network bandwidth and/or cloud costs.
...
- *Use of Application Control shared rulesets through a proxy connection*: If you will use shared Application Control rulesets and [agents connect through a proxy](../../configure-proxies/configure-proxies-server-workload), you might want to add more relays to handle large rulesets and improve performance. See [Deploy Application Control rulesets via relays](../../../configure-protection-modules/configuring-application-control/use-the-api-to-create-shared-and-global-rulesets/api-shared-global-rulesets) and [Server & Workload Protection Sizing](../../manage-agents-protected-computers/configure-agent-version-control/agent-platform-compatibility/sizing-server-workload).
...
2.  Make sure you allow inbound and outbound communication to and from the relay on the appropriate port numbers. See [Server & Workload Protection port numbers](../../../troubleshooting/swp-port-numbers).

3.  Deploy an agent on the chosen computer. See [Get Deep Security Agent software](../../manage-agents-protected-computers/get-agent-software) and [Install the agent](../../manage-agents-protected-computers/install-the-agent).
...
You must indicate which relay group each agent should use. Either assign each agent to a relay group manually, or set up an [event-based task](../../automate/automate-using-the-console/autotasks-computer-changed-event) to assign new agents automatically.
...
2.  Under *Software Updates*, in *Alternate software update distribution server(s) to replace Deep Security Relays*, type:
    ...
    where `<IP>` is the private network IP address of the relay, and `<port>` is the [relay port number](../../../troubleshooting/swp-port-numbers)
