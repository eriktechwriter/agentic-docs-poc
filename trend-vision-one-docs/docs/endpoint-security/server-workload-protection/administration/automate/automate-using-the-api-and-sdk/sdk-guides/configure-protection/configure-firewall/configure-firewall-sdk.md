...
When designing the module's behavior and implementing it using the API, use the background information and guidance that is provided in [About Firewall](../../../../../../configure-protection-modules/configure-firewall/firewall).
...
- `PolicySettings`: Policy settings include many Firewall-related settings that control the runtime behavior of the module, such as the behavior of reconnaissance scans, network engine mode (tap or inline), network engine settings, and event management.Configure Firewall-related policy settings as described in [Configure policy and default policy settings](../create-and-configure-a-policy/create-configure-policy).
...
The `moduleStatus` property is read-only. It provides the runtime status of the Firewall module. (See [Report on Computer Status](../../maintain-protection/report-on-computer-status/report-computer-status)).
...
4.  Use a `PoliciesApi` object to add or update the policy on Server & Workload Protection.
    ...
    :::tip
    If you only need to set a single Firewall-related policy setting, see [Configure a single policy or default policy setting](../create-and-configure-a-policy/create-configure-policy).
    :::
...
Next, create a `PolicySettings` object to configure Firewall-related settings. (For detailed information about policy settings, see [Configure policy and default policy settings](../create-and-configure-a-policy/create-configure-policy).) For example, you can enable reconnaissance scans:
...
For information about authenticating API calls, see [Authenticate with Server & Workload Protection](../../../send-your-first-request-using-api).
...
2.  Set the rule properties. The properties are as described in [Create a firewall rule](../../../../../../policies/define-rules-lists-and-other-common-objects-used-by-policies/create-firewall-rule). You can use the API to create related objects that can be used with multiple rules, such as MAC lists, rule contexts, and schedules. See [Create and Modify Lists](../create-modify-lists) and [Create and Configure Schedules](../create-configure-schedules).
...
:::tip
Although Log Inspection rules have different properties than Firewall rules, the way you create the rules are similar. You might find the [Create a basic Log Inspection rule](../configure-log-inspection/config-log-inspection-swp) example helpful.
:::
...
Therefore, these properties are not configurable using the API or an SDK. You must use the Server & Workload Protection console to configure these settings. See [Define stateful firewall configurations](../../../../../../policies/define-rules-lists-and-other-common-objects-used-by-policies/define-stateful-firewall-configurations/define-stateful-firewall).
