...
Configure the Integrity Monitoring module to define its behavior for a policy. When designing the module’s behavior and implementing it using the API, use the same background information and guidance that is provided in [About Integrity Monitoring](../../../../../../configure-protection-modules/configure-integrity-monitoring/integrity-monitoring).
...
- `PolicySettings`: Policy settings include many Integrity Monitoring-related settings that control the runtime behavior of the module, such as the application of [recommendation scans](../../../../../../computers/recommendation-scans), whether real-time scan is enabled, performance-related settings, and the Syslog configuration to use. (See [Configure policy and default policy settings](../create-and-configure-a-policy/create-configure-policy).)
...
The `moduleStatus` property is read-only. It provides the runtime status of the Integrity Monitoring module. (See [Report on Computer Status](../../maintain-protection/report-on-computer-status/report-computer-status).)
...
2.  Create a `PolicySettings` object to configure runtime settings of the module. (See [Configure policy and default policy settings](../create-and-configure-a-policy/create-configure-policy).)
...
2.  Set the rule properties. Rules are described in [Create an Integrity Monitoring rule](../../../../../../configure-protection-modules/configure-integrity-monitoring/create-an-integrity-monitoring-rule/create-integrity-monitoring-rule).
...
:::tip
Although Log Inspection rules have different properties than Integrity Monitoring rules, the way you create the rules are similar. You might find the [Create a basic Log Inspection rule](../configure-log-inspection/config-log-inspection-swp) and [Create a Log Inspection rule using XML](../configure-log-inspection/config-log-inspection-swp) examples helpful.
:::
