...
When designing the module's behavior and implementing it using the API, use the background information and guidance that is provided in [About Device Control](../../../../../../configure-protection-modules/configure-device-control).
...
Configure Device Control related policy settings as described in [Configure policy and default policy settings](../create-and-configure-a-policy/create-configure-policy).
...
The `moduleStatus` property is read-only. It provides the runtime status of the Device Control module. (See [Report on Computer Status](../../maintain-protection/report-on-computer-status/report-computer-status)).
...
4.  Use a `PoliciesApi` object to add or update the policy in Server & Workload Protection.
    ...
    :::tip
    If you only need to set a single Device Control related policy setting, see [Configure a single policy or default policy setting](../create-and-configure-a-policy/create-configure-policy).
    :::
...
Next, create a `PolicySettings` object to configure Device Control related settings. (For detailed information about policy settings, see [Configure policy and default policy settings](../create-and-configure-a-policy/create-configure-policy).) For example, you can block USB mass storage access:
