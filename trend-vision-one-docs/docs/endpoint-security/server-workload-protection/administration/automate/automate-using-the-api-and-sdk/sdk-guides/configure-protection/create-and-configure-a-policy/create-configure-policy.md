...
- [Configure Firewall](../configure-firewall/configure-firewall-sdk)
- [Configure Intrusion Prevention](../configure-intrusion-prevention/configure-intrusion-prevention-sdk)
- [Configure Anti-Malware](../configure-anti-malware/configure-anti-malware-sdk)
- [Configure Web Reputation](../configure-web-reputation/configure-wrs-server-workload)
- [Configure Device Control](../configure-device-control/configure-device-control-sdk)
- [Configure Application Control](../configure-application-control/config-app-control-policy)
- [Configure Integrity Monitoring](../configure-integrity-monitoring/configure-integrity-monitoring-sdk)
- [Configure Log Inspection](../configure-log-inspection/config-log-inspection-swp)
:::

Before you use the API, you should understand the essential concepts about policies. For background information, see [Create policies to protect your computers and other resources](../../../../../../policies/create-policies/create-policies).
...
For information about searching, see [Search for Resources](../../../search-for-resources). For information about authenticating API calls, see [Authenticate with Server & Workload Protection](../../../send-your-first-request-using-api).
...
:::tip
You can override a policy at the computer level. See [Configure Computers to Override Policies](../override-policies-on-a-computer/override-policies-computer).
:::
...
For a list of policy and default policy settings, see [Default policy, policy, and computer settings](../../../settings-reference) in the Settings Reference.
...
For more information about the policy hierarchy and inheritance, see [Policies, inheritance, and overrides](../../../../../../policies/policies-inheritance-overrides).
...
The settings of the `DefaultPolicySettings` and `PolicySettings` classes are identical, with a few exceptions. (See [Default policy, policy, and computer settings](../../../settings-reference) in the Settings Reference.)
...
When you retrieve a setting value, identify the setting by name. The value is returned as a `SettingValue` object. For a list of policy and default policy settings, see [Default policy, policy, and computer settings](../../../settings-reference) in the Settings Reference.
...
For an example of configuring a system setting, see [Configure Server & Workload Protection System Settings](../../deploy-server-workload-protection/configure-server-workload-protection-system-settings/configure-system-settings).
...
For example, the [Set the real time Anti-Malware scan configuration](../configure-anti-malware/configure-anti-malware-sdk) example overrides the realTimeScanConfigurationID for a policy. To reset the property to inherit the value from the parent policy, set `realTimeScanConfigurationID` to `0`.
...
To reset several policy settings at the same time, use a `PolicySettings` object with `PoliciesApi` to set the value of the settings to an empty string. For example, the [Configure Firewall](../configure-firewall/configure-firewall-sdk) example overrides the value of the `FirewallSettingReconnaissanceEnabled` setting for a policy. To reset the setting to inherit the value from the parent policy, you set the value to `""`.
...
For example, the [Turn on Application Control](../configure-application-control/config-app-control-policy) example overrides the status property of the Application Control module to `on` for a policy. To reset the setting to inherit the value from the parent policy, you set the status value to `inherited`.
