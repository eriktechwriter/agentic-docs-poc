...
When you enable agent-initiated activation (AIA), instead of Server & Workload Protection contacting the agents directly, the agents initiate communication with Server & Workload Protection and establish an encrypted TCP connection over the Server & Workload Protection heartbeat [port number](../../../troubleshooting/swp-port-numbers) (443).
...
:::note
For a full description of each AIA setting, see the [Agent-Initiated Activation](../agent-settings) section of [Agent settings](../agent-settings).
:::
...
If all the agents will use the same policy, you can assign the policy in the deployment script as part of the next step. If groups of agents need to use different policies, [create an event-based task to assign the policies](../../automate/automate-using-the-console/autotasks-computer-changed-event) before proceeding with the next step.

## Use a deployment script to activate the agents {#use-a-deployment-script-to-activate-the-agents}

See the [Generate a deployment](../../automate/automate-using-the-console/use-deployment-scripts-to-add-and-protect-computers/deployment-scripts-add-computers) section of [Generate a deployment script](../../automate/automate-using-the-console/use-deployment-scripts-to-add-and-protect-computers/deployment-scripts-add-computers) to learn how to use a deployment script to activate the agents. If you are assigning a policy during deployment script configuration, you'll select it from the Security Policy list.
