...
1.  Forward security events to external storage. See [Forward events to an external Syslog or SIEM server](../forward-events-to-a-syslog-or-siem-server/forward-server-workload-protection-events-to-a-syslog-or-siem-server/forward-syslog-siem-server).

2.  Set thresholds in the log inspection module for event storage or event forwarding. *Severity clipping* allows you to send events to a Syslog server (if enabled) or to store events based on the severity level of the log inspection rule. See [Thresholds for Event Storage or Event Forwarding](../../../configure-protection-modules/configure-log-inspection/set-up-log-inspection/set-up-log-inspection).
