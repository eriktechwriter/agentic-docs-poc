...
For an overview of Application Control, see [Lock down software with Application Control](../../configure-protection-modules/configuring-application-control/about-application-control/application-control). For initial configuration instructions, see [Set up Application Control](../../configure-protection-modules/configuring-application-control/set-up-application-control/set-up-application-control).
...
You can configure some of which Application Control event logs are recorded, and which are [forwarded to external SIEM systems or syslog servers](../../events-reports/about-server-workload-protection-event-logging/forward-events-to-a-syslog-or-siem-server/forward-server-workload-protection-events-to-a-syslog-or-siem-server/forward-syslog-siem-server).
...
3.  If you want to record event logs for that type of event, select *Record*.
    ...
    When those events occur, they appear on **Events → Reports → Events → System Events**. Logs are kept until they meet maximum log age criteria. For details, see [Event collection in Server & Workload Protection](../../events-reports/about-server-workload-protection-event-logging).
...
5.  If you use an external SIEM, you may need to load the list of possible Application Control event logs, and indicate what action to take. For a list of Application Control events, see [Application Control events](../../events-reports/about-server-workload-protection-event-logging/application-control-events).
...
- *System event:* An audit event that provides a history of configuration changes or software updates. To see system events click **Events & Reports → Events → System Events**. For a list, see [System events](../../events-reports/about-server-workload-protection-event-logging/system-events).
...
To configure which Application Control events or severity levels cause an alert, go to the *Alerts* tab, click the *Configure Alerts* button, and then select an event and double-click *Properties*. For details, see [Configure alerts](../configure-alerts/configure-alerts).
