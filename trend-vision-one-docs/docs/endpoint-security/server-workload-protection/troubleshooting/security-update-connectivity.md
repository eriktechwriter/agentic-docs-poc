...
1.  To verify that both a route exists and that the [relay port number](./swp-port-numbers) is open, enter the command:
...
3.  If you use a proxy server, in Server & Workload Protection, confirm that the [proxy settings](../administration/configure-proxies/configure-proxies-server-workload) are correct.
