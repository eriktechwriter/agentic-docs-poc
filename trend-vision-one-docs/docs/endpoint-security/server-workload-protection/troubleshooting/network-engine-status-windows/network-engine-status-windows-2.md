...
Network Engine Status warnings are a collection of warnings/errors that might appear in the [Status area](../../policies/overview-section-computer-editor) of a computer. They appear when the agent raises an event about the *Trend Micro LightWeight Filter Driver*, and the [Network Engine Status Check](../../policies/network-engine-settings) feature is enabled.
...
:::note
Agents will display more security events for each affected network interface. See [Driver-Related Events](../../events-reports/about-server-workload-protection-event-logging/system-events/agent-events) table for the list of agent events.
:::
