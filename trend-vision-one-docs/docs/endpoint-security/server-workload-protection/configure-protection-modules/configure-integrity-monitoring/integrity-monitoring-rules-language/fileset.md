...
:::note
The Integrity Monitoring module scans for unexpected changes to [directories](./directoryset), [registry values](./registryvalueset), [registry keys](./registrykeyset), [services](./serviceset), [processes](./processset), [installed software](./installedsoftwareset), [ports](./portset), [groups](./groupset), [users](./userset), files, and the [WQL](./wqlset) query statement on agents. To enable and configure Integrity Monitoring, see [Set up integrity monitoring](../set-up-integrity-monitoring/set-up-integrity-monitoring).
:::
...
See [Integrity monitoring rules language](./integrity-monitoring-language-swp) for a general description of Include and Exclude for their allowed attributes and sub elements. Only information specific to includes and excludes relating to the FileSet Entity Set class are included here.
