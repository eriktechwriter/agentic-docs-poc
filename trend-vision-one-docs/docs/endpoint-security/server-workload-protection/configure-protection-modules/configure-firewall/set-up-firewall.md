...
1.  In the Computer or Policy editor, go to **Firewall → General**.
    ...
    :::note
    For information on enabling Firewall for containers, see [Apply your firewall settings](../../policies/define-rules-lists-and-other-common-objects-used-by-policies/manage-container-protection/manage-container-protection).
    :::
...
- *Frame type:* IP
- *Protocol:* TCP
- *Packet Destination Port:* [Agent's listening port for heartbeats](../../troubleshooting/swp-port-numbers)
...
For more information on how to create a Firewall rule, see [Create a Firewall rule](../../policies/define-rules-lists-and-other-common-objects-used-by-policies/create-firewall-rule).

### Firewall rule priorities {#firewall-rule-priorities}

Rule priority determines the order in which filters are applied. This means that high priority rules get applied before low priority rules. When actions share the same priority, the orders of precedence for rules are: Bypass, Force Allow, and then Deny. However, a Deny action with a higher priority will take precedence over a Bypass action with a lower priority. For more information on how rule priorities and actions determine processing order, see [Firewall rule actions and priorities](../../policies/define-rules-lists-and-other-common-objects-used-by-policies/firewall-rule-actions-and-priorities/firewall-rule-actions-priorities).
...
For information on how to handle reconnaissance warnings, see [Warning: Reconnaissance Detected](../../events-reports/about-server-workload-protection-event-logging/intrusion-prevention-events/warning-reconnaissance-detected).
