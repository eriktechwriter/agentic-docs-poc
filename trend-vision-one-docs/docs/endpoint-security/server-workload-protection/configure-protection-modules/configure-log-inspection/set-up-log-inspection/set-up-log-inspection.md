...
For an overview of the log inspection module, see [About Log Inspection](../log-inspection).
...
Run a [recommendation scan](../../../computers/recommendation-scans) on the computer for recommendations about which rules to apply.

## Apply the recommended log inspection rules {#apply-the-recommended-log-inspection-rules}

Server & Workload Protection ships with many pre-defined rules covering a wide variety of operating systems and applications. When you run a [recommendation scan](../../../computers/recommendation-scans), you can choose to have Server & Workload Protection [automatically implement recommendations](../../../computers/recommendation-scans/auto-imp-recommendations) or you can choose to [manually select and assign the rules](../../../computers/recommendation-scans/manual-assign-rules).

Although Server & Workload Protection ships with log inspection rules for many common operating systems and applications, you also have the option to create your own custom rules. To create a custom rule, you can either use the Basic Rule template, or you can write your new rule in XML. For information on how to create a custom rule, see [Define a log inspection rule for use in policies](../define-inspection-rule).
...
When an event triggers a log inspection rule, Server & Workload Protection logs the event. You can view these Log Inspection Events under **Events & Reports** and **Policy editor** (see [Log inspection events](../../../events-reports/about-server-workload-protection-event-logging/log-inspection-events)). Depending on the severity of the event, you may send the event to a syslog server (see [Forward Server & Workload Protection events to an external syslog or SIEM server](../../../events-reports/about-server-workload-protection-event-logging/forward-events-to-a-syslog-or-siem-server/forward-server-workload-protection-events-to-a-syslog-or-siem-server/forward-syslog-siem-server)) or store events in the database using the severity clipping feature.
