...
:::note
The *Advanced* tab also contains *Events* settings. For information on those settings, see [Limit log file sizes](../events-reports/about-server-workload-protection-event-logging/log-and-event-storage/log-event-storage). It also contains the *Generate an Alert when Agent configuration package exceeds maximum size* setting, which controls the display of the "Agent configuration package too large" setting.
:::
...
- *Network Engine Status Check*: This setting determines if the agent will monitor the status of the Network Engine. This is enabled by default, but can be disabled. For related events, see [Network Engine Status (Windows)](../events-reports/about-server-workload-protection-event-logging/system-events/network-engine-status-windows/network-engine-status-windows).
...
- *Anti-Evasion Posture*: The anti-evasion setting controls the network engine handling of abnormal packets that may be attempting to evade analysis. For details, see [Configure anti-evasion settings](../configure-protection-modules/configure-intrusion-prevention/configure-anti-evasion-settings).
