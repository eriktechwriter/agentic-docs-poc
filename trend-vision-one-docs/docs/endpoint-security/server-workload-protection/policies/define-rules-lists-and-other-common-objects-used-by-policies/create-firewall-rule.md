...
:::note
This article specifically covers how to create a firewall rule. For information on how to configure the firewall module, see [Set up the Server & Workload Protection firewall](../../configure-protection-modules/configure-firewall/set-up-firewall).
:::
...
2.  Select the *Action* that the rule should perform on packets. You can select from one of the following five actions:
    ...
    :::tip
    For an example of how to create and use a bypass rule for trusted sources in a policy, see [Allow trusted traffic to bypass the firewall](./trusted-traffic-bypass-firewall).
    :::
...
3.  Select the *Priority* of the rule. The priority determines the order in which rules are applied. If you have selected "force allow", "deny", or "bypass" as your rule action, you can set a priority of 0 (low) to 4 (highest). Setting a priority allows you to combine the actions of rules to achieve a cascading rule effect.
    ...
    *Log only* rules can only have a priority of *4*, and *Allow* rules can only have a priority of *0*.
    ...
    For detailed information on how actions and priority work together, see [Firewall rule actions and priorities](./firewall-rule-actions-and-priorities/firewall-rule-actions-priorities).
...
:::tip
You can use a previously created [IP](./about-common-objects/create-a-list-of-ip-addresses-for-use-in-policies/create-ip-policies), [MAC](./about-common-objects/create-a-list-of-mac-addresses-for-use-in-policies/create-mac-addresses-policies) or [port](./about-common-objects/create-a-list-of-ports-for-use-in-policies/create-list-ports-for-use-policies) list.
:::
...
For more information on how to do so, see [Define a schedule that you can apply to rules](./about-common-objects/define-schedule-rules).
