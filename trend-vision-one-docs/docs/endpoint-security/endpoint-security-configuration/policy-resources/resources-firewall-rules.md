...
Firewall rules are a policy resource you can configure to manage how [Firewall](../endpoint-security-policies-unified-assignment/policies/policies-firewall) handles specific connection types, usually to override the basic Firewall settings. Firewall rules can allow, block, and log internet connections.
...
8.  Specify the **Packet source**.
    ...
    1.  If you want to monitor specific IP addresses, select **Use IP list** then select an **IP list**.
        ...
        To configure IP lists, see [IP Lists](./resources-ip-lists).

    2.  If you want to monitor specific ports, select **Use port list** then select a **Port list**.

        If you select **Any** for **Protocol**, then **Port** is set to **Any** and cannot be changed. To configure port lists, see [Port Lists](./resources-port-lists).
