...
:::warning[Important]
- Policies, Threat Prevention, and the Intrusion Prevention module are "Pre-release" features and are not considered an official release. Please review the [Pre-Release Disclaimer](../../../../privacy-and-personal-data-collection-disclosure/pre-release-disclaimer) before using the feature.
...
- Intrusion Prevention uses IP lists for configuring IP address exclusions. Configure [IP lists](../../policy-resources/resources-ip-lists) in policy resources before configuring Intrusion Prevention.
...
3.  Configure **Recommendation settings**.
    ...
    :::warning[Important]
    **Use Recommendation Scan to dynamically apply rules to each endpoint** only supports endpoints with Server & Workload Protection features. If applied to endpoints with Standard Endpoint Security features, the agent only applies the **Core** ruleset. Support for Standard Endpoint Security is coming soon.
...
5.  To exclude trusted IP addresses from Intrusion Prevention scans and monitoring, select an IP list for **IP address exclusions**.
    ...
    You can create and configure IP lists in Policy Resources. For more information, see [IP lists](../../policy-resources/resources-ip-lists).
