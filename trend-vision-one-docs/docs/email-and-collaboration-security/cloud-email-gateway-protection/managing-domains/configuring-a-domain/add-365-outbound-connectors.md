...
4.  Do the following to add an Outbound Connector to Microsoft 365:
    ...
    2.  In the **Connection from** field, select **Office 365**.

    3.  In the **Connection to** field, select **Partner organization**.
...
5.  Add an email flow rule to use the outbound connector you created.
    ...
    4.  Under **Apply this rule if...**, select **The recipient**, **is external/internal**, and **Outside the organization**, and then click **Save**.

    5.  Under **Do the following...**, select **Redirect the message to** and then **the following connector** and choose the outbound connector you created for message redirection.

    6.  Configure the remaining fields if necessary; otherwise, keep the default settings for them.
