...
4.  Do the following to add an Inbound Connector to Microsoft 365:
    ...
    2.  In the **Connection from** field, select **Partner organization**.

    3.  In the **Connection to** field, select **Office 365**.
    ...
    8.  Select **By verifying that the IP address of the sending server matches one of the following IP addresses, which belong to your partner organization**, and add the following Cloud Email Gateway Protection IP addresses:
    ...
    10. Select **Reject email messages if they aren't sent over TLS**, and then click **Next**.
