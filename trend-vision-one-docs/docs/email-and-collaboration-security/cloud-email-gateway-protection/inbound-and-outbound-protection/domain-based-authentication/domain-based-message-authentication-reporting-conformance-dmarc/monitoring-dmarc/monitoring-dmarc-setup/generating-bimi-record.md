...
4.  Check whether a DMARC record has been published in DNS for the domain or whether the DMARC policy for the domain and the organizational domain (if different) is enforced. If not, [generate a DMARC record or update the current DMARC record](./generating-dmarc-record), and then publish it in DNS.
