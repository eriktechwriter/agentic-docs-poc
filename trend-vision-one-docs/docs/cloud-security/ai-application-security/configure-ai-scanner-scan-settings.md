...
1.  Create an application programming interface (API) key for your AI model.
    ...
    3.  Specify a name, user role, and expiration time for the API key.
        ...
        :::warning[Important]
        When adding the API key, you must select a **Role** with permission to run scans in AI Scanner. If your organization does not have an existing user role with the necessary permissions, create a [custom role](../../administration/user-roles/custom-user-roles-trend-vision-one) or contact your administrator.
        :::

        For more information, see [API Keys](../../administration/api-keys).

    4.  Click **Add**.

    5.  Copy and retain the API key.

2.  Download and install the [Trend Micro Artifact Scanner CLI](../container-security/log/container-image-scanning/trend-micro-artifact-scanner-tmas/integrate-trend-micro-artifact-scanner-tmas-into-a-cicd-pipeline).
...
4.  View the results of the scan in AI Scanner.
    ...
    For more information on the results available in AI Scanner, see [AI Scanner scan results](./ai-scanner-scan-results).
