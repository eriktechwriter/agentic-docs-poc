...
- File Security SDK is an scanner app that can assess files for malware and display real-time results so you can immediately identify and address likely security issues in your files. File Security SDK can leverage the power of [predictive machine learning (PML)](../../email-and-collaboration-security/cloud-email-gateway-protection/configuring-policies/about-policy-rule-scanning-criteria/configuring-virus-scan-criteria/pml-email-security) to match novel variants against known hazards.
...
For more information, see [What is File Security](./what-is-file-security).
...
- [Storage](./file-security-storage) – Deploy through the Cloud Accounts app to selected regions. Then, on the Inventory tab, choose which buckets you want to protect and turn on EventBridge for those buckets.

- [SDK](./file-security-sdk) – Deploy using either a software development kit (SDK) or the command line interface (CLI) to scan files.
...
At this time, File Security Storage works with Amazon Web Service (AWS) storage. See [Deploying for AWS storage](./file-security-storage/file-security-storage-for-aws/deploy-fss-new-account).
