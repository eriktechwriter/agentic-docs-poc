...
Compliance scan can run multiple times per day, thus an average is calculated for all the Compliance scan runs. For each account, the individual scores of each category is obtained by the following formula for every bot run.
