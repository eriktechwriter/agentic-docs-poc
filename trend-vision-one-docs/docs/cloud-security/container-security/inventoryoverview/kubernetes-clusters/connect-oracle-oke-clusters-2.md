---
slug: trend-vision-one-connect-oracle-oke-clusters-2
sidebar_position: 7
title: Connect Oracle Cloud OKE clusters
---

Container Security supports protection on connected Oracle Cloud OKE containers.

:::warning[Important]
- Ensure that your [Kubernetes environments meet the necessary requirements](../../getting-started-with-container-security/kubernetes-cluster-security/kubernetes-system-requirements-for-container-security) for Container Security.

- For users that want to use the [Isolate Container task](../../../../workflow-and-automation/response-management/response-actions/isolate-container-task), you must ensure that your cluster implements the **Calico** network policy.
:::
...
4.  Specify a unique name for the cluster.
...
6.  If you want Container Security to send data to Cloud Risk Management and receive [CREM Risk Insights](../../../../cyber-risk-exposure-management/cyber-risk-overview), select **Map to cloud account**.

    1.  In the drop-down, select **Oracle Cloud**.

    2.  In another browser tab, sign in to the Oracle Cloud account that hosts the cluster.

    3.  Copy and paste the Cluster ID for the cluster you want to protect, and enter the corresponding values under **Map to cloud account**.
...
7.  If you have already created a policy that you want to use to protect the Kubernetes cluster, select the policy name from the **Policy** drop-down.
    ...
    You can [create a Kubernetes policy](../../configuration/policy/kubernetes-protection-policies) and assign the policy after connecting the cluster.
...
10. If you already know the type of security features you want to enable on the cluster, turn on the desired features.
    ...
    - **Runtime Secret Scanning**: Provides detection of secrets in your running containers and ensures that any secret leaks in production containers are detected quickly.
...
12. For users deploying Container Security protection the cluster for the first time:
    ...
    2.  To enable automated cluster registration, [create an API key](./obtain-apikey-auto-cluster-reg) and enter `true` for `clusterRegistrationKey`, as shown in the example below.
...
13. To update an existing deployment, copy the entire `helm get values --namespace trendmicro-system trendmicro | helm upgrade \` script in the last input field and execute the Helm script on your cluster.
