...
- Before connecting an Amazon EKS Fargate pod, ensure that the container meets the necessary [system requirements for EKS Fargate](./amazon-eks-fargate-sys-reqs) deployments.
:::
...
8.  To ensure that Container Security does not impact any of the following Kubernetes management systems, select the systems in the **Namespace Exclusions** dropdown menu.
    ...
    :::note
    Options with an asterisk (\*) represent sets of related namespaces. For details, see [Grouped namespaces](../grouped-namespaces).
    :::
...
12. For users deploying Container Security protection the cluster for the first time:
    ...
    3.  To enable automated cluster registration, [create an API key](../../../inventoryoverview/kubernetes-clusters/obtain-apikey-auto-cluster-reg) and enter `true` for `clusterRegistrationKey`, as shown in the example below.
...
- [Amazon EKS Fargate system requirements](./amazon-eks-fargate-sys-reqs)
