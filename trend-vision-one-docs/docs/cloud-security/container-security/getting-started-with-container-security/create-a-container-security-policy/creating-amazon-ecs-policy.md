...
:::warning[Important]
Policy configuration for Kubernetes clusters differs greatly from a Amazon ECS environment. To properly configure Kubernetes protection policies, see [Creating a Kubernetes protection policy](./kubernetes-prot-policy).
:::
...
8.  Define the cluster-wide rules that apply while a pod is running by clicking the **Runtime** tab.
    ...
    The runtime policy consists of the [rulesets you create](../runtime-security-ruleset) on the **Rulesets** tab.
