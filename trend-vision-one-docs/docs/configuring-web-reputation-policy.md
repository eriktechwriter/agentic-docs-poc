...
4.  Select **Check HTTPS URLs**.
    ...
    - HTTPS URL scanning also supports the HTTP/2 protocol. Before Web Reputation can check HTTPS or HTTP/2 URLs, you must configure some prerequisite settings for different browsers.
      ...
      For more information, see [HTTPS URL Scan Support](https-url-scan-support.md).

    - Starting from Trend Vision One Endpoint Security agent version 202510 and later (Standard Endpoint Protection component version 14.0.0.20315), enabling the **Check HTTPS URLs** option will download and install the Trend Micro Toolbar for Enterprise browser extension on endpoints with Google Chrome and Microsoft Edge. The browser extension helps enhance security features such as blocking URLs and URLs added to your Suspicious Object List.

      The Trend Micro Toolbar for Enterprise only supports Google Chrome and Microsoft Edge on Windows 64-bit endpoints. Browser extension does not support Windows Server 2008 R2 (64-bit).

      The browser extension uses the following services to communicate with the Trend Vision One Endpoint Security agent:

      - Trend Micro Network Service (tm_netsrv.exe)

      - Trend Micro Web Reputation App (dsa-wrs-app.exe)

    - Disabling the **Check HTTPS URLs** option uninstalls the Trend Micro Toolbar for Enterprise browser extension from endpoints assigned to the policy.
    :::
...
- [HTTPS URL Scan Support](https-url-scan-support.md)
