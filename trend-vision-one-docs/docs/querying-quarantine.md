...
3.  Type your search criteria into one or more of the following fields:
    ...
    You can specify up to 10 recipient or sender email addresses. Separate multiple entries by pressing the **ENTER** or **TAB** key, or using a semicolon (;).

    A recipient or sender can be a specific email address or specific addresses from specific managed domains. Specify an exact email address or use wildcards (\*) to substitute any characters in a search.

    - Query a specific email address by typing that email address.

    - In the general format of an email address (local-part@domain), query specific addresses by using an asterisk (\*) to either or both of the local-part and domain parts. For example, using `*@example.com` searches for all email addresses in the `example.com` domain and displays the query result if `example.com` is within your management scope.
    ...
    **Format Examples for Mail Tracking and Quarantine Query**
    ...
    <td><p><code>name@example.*</code></p></td>
    </tr>
    <tr>
    ...
    Use wildcards (\*) strategically to expand or narrow your search results. For example, put a wildcard (\*) in the domain part to search by a particular user account on all managed domains or in the local part to match all accounts on a particular managed domain.
