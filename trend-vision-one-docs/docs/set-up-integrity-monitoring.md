...
You can enable Integrity Monitoring in [policies](policies-server-workload.md) or at the computer level. Enabling Integrity Monitoring involves several procedures:
...
Run a [recommendation scan](recommendation-scans-intro.md) on the computer to get recommendations for appropriate rules. Recommended Integrity Monitoring rules may result in too many monitored entities and attributes. The best practice is to decide what is critical and should be monitored, then create custom rules or tune the predefined rules. Pay extra attention to rules that monitor frequently-changed properties such as process IDs and source port numbers because such rules can be noisy and may need some tuning.
...
When you run a [recommendation scan](recommendation-scans-intro.md), you can have Server & Workload Protection automatically implement the recommended rules or you can manually assign rules. You can edit a rule locally, so the changes apply only to that computer or policy, or globally, so the changes apply to all policies or computers that use the rule. You can also create custom rules to monitor for specific changes that concern your organization, such as a new user being added or new software being installed. For information on how to create a custom rule, see [Integrity monitoring rules language](integrity-monitoring-language-swp.md).
...
For more information on event tagging, see [Apply tags to identify and group events](apply-tags-identify-group-events.md).
