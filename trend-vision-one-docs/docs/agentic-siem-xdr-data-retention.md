...
To extend data retention, you can [allocate credits to Agentic SIEM](credit-req-for-apps-services.md) and select retention periods for individual data sources in **Data Source and Log Management → Data sources and retention → Agentic SIEM & XDR**. Data for most data sources can be retained for up to two years. You can only change the retention period for a data source once per day.
