---
slug: trend-vision-one-add-user
sidebar_position: 2
title: Add user
---
...
Cloud Risk Management provides management tools to give your organization's users access to [Cloud Accounts](../../manage-cloud-accounts/cloud-accounts-cloud-posture-setup) to fit into your business' workflow and processes. Access to Cloud Risk Management is provisioned according to the needs of each staff member using [Roles and Entitlements](../roles-and-permissions/roles-permissions). Users can be granted access to one or many accounts, with the ability to restrict access to read only or administrator for each account. This separation of access ensures staff only gains visibility of infrastructure and best practice violations specific to their duties.
...
Once an organisation has been set up, an [Administrator](../roles-and-permissions/roles-permissions) can invite new users. Currently, we do not allow users creation in bulk, however there is no limit on the number of users an Administrator role can invite.

Input details of the new user and *Invite user.* You can also decide whether the new user will be an [Administrator.](../roles-and-permissions/roles-permissions) Invited users will receive an invitation email to join the Cloud Risk Management platform.

Note: Administrator role can invite users only after an account has been added.

### Manage user {#manage-user}

You can *Configure* any user's access level at account level by assigning a different role. While a user can have access to any number of accounts, the Custom Role provides you with more flexibility to assign separate access for each account. For more information on roles and associated access levels, see [Roles and Entitlements](../roles-and-permissions/roles-permissions).
