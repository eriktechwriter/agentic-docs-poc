...
Cloud Risk Management offers multiple filters when viewing rules and checks on available [reporting tools](../../../reports/rules-status-reports). You can view the filtered results by *Rule,* *Resource,* or [Standards or Frameworks](../../../compliance/compliance-reports).

Using the filter, you can take several actions such as configuring each rule according to your requirements, resolving failures, viewing only [suppressed](../../checks/rules-suppress-check) or unsuppressed checks, Resource id, message, only show checks, generating a [report](../../../reports/rules-status-reports).
