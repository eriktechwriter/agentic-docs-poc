...
- File Security SDK is an scanner app that can assess files for malware and display real-time results so you can immediately identify and address likely security issues in your files. File Security SDK can leverage the power of [predictive machine learning (PML)](pml-email-security.md) to match novel variants against known hazards.
...
For more information, see [What is File Security](what-is-file-security.md).
...
- [Storage](cloud-storage-scanning.md) – Deploy through the Cloud Accounts app to selected regions. Then, on the Inventory tab, choose which buckets you want to protect and turn on EventBridge for those buckets.

- [SDK](sdk-or-cli-scanning.md) – Deploy using either a software development kit (SDK) or the command line interface (CLI) to scan files.
...
At this time, File Security Storage works with Amazon Web Service (AWS) storage. See [Deploying for AWS storage](deploy-fss-new-account.md).
