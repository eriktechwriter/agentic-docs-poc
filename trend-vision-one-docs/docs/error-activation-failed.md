...
When the agent uses agent-initiated communication, you need to activate the agent from the agent computer. (See [Activate an agent](command-line-basics.md).)
...
2.  Ensure that the ports are open between Server & Workload Protection and the agent. (See [Port numbers](swp-port-numbers.md) and [Define a firewall rule](create-firewall-rule.md).)
...
If you are a Server & Workload Protection customer, we recommend that you always use agent-initiated activation. To learn how to configure policy rules for agent-initiated communication and deploy agents using deployment scripts, see [Activate and protect agents using agent-initiated activation and communication](agent-initiated-activation.md).
...
Review [Deploy agents to your EC2 instances](install-agent-amazon-ec2-workspaces.md).
...
For details on ports, see [Port numbers](swp-port-numbers.md).
...
If you are using a proxy, in the console go to **Support → Deployment Scripts** and update the fields with your proxy, then reactivate the agent. For more information, see [Use deployment scripts to add and protect computers](deployment-scripts-add-computers.md).

### Reinstallation required {#reinstallation-required}

If the agent is not activating, you may need to [Uninstall the agent](uninstall-swp-agent.md), then [reinstall the agent](install-agent.md).
