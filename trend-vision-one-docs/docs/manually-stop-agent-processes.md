...
:::note
This FAQ is about how to manually terminate agent processes when using the agent interface to shut down the Trend Vision One Endpoint Security agent on a Windows endpoint. For information about shutting down the agent, see [Agent Interface](policies-agent-interface.md).

For steps on restarting the agent and agent services, see [How to restart the Trend Vision One Endpoint Security agent](restart-security-agent.md).
