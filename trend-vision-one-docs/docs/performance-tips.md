...
The following code, from an [example](report-computer-status.md) in the [Report on Computer Status](report-computer-status.md) guide, obtains a list of computers that include the computer status property.
...
:::tip
For more information, see the [About the Overrides Parameter](overrides-parameter.md) guide.
:::
...
As described in [Create and Configure a Policy](create-configure-policy.md), there are two ways that you can interact with policy settings:
...
Similarly, use the `PoliciesApi` class to interact directly with individual default policy settings, use the `ComputersApi` class for individual computer settings, and use the `SystemSettingsApi` class for individual system settings (see [Configure Computers to Override Policies](override-policies-computer.md) and [Configure Server & Workload Protection System Settings](configure-system-settings.md)).
...
For more information, see the "Limit search results and paging" section of the [Search for Resources](search-for-resources.md) guide.
