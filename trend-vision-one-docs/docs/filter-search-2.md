...
Cloud Risk Management offers multiple filters when viewing rules and checks on available [reporting tools](rules-status-reports.md). You can view the filtered results by *Rule,* *Resource,* or [Standards or Frameworks](compliance-reports.md).

Using the filter, you can take several actions such as configuring each rule according to your requirements, resolving failures, viewing only [suppressed](rules-suppress-check.md) or unsuppressed checks, Resource id, message, only show checks, generating a [report](rules-status-reports.md).
