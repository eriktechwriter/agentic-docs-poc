...
Configure the Integrity Monitoring module to define its behavior for a policy. When designing the module’s behavior and implementing it using the API, use the same background information and guidance that is provided in [About Integrity Monitoring](integrity-monitoring.md).
...
The `moduleStatus` property is read-only. It provides the runtime status of the Integrity Monitoring module. (See [Report on Computer Status](report-computer-status.md).)
...
2.  Create a `PolicySettings` object to configure runtime settings of the module. (See [Configure policy and default policy settings](create-configure-policy.md).)
...
2.  Set the rule properties. Rules are described in [Create an Integrity Monitoring rule](create-integrity-monitoring-rule.md).
...
:::tip
Although Log Inspection rules have different properties than Integrity Monitoring rules, the way you create the rules are similar. You might find the [Create a basic Log Inspection rule](config-log-inspection-swp.md) and [Create a Log Inspection rule using XML](config-log-inspection-swp.md) examples helpful.
:::
