...
12. Use the CLI to configure the required network settings.
    ...
    If your organization does not use a DHCP server, manually configure the static IP address, static route, and DNS server for your interface. For more information on available commands, see [Private Access Connector CLI commands](private-access-connector-cli.md).
