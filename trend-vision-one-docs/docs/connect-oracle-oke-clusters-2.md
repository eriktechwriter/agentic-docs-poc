...
:::warning[Important]
- Ensure that your [Kubernetes environments meet the necessary requirements](kubernetes-system-req.md) for Container Security.

- For users that want to use the [Isolate Container task](isolate-container-task.md), you must ensure that your cluster implements the **Calico** network policy.
:::
...
6.  If you want Container Security to send data to Cloud Risk Management and receive [CREM Risk Insights](executive-dashboard.md), select **Map to cloud account**.
...
7.  If you have already created a policy that you want to use to protect the Kubernetes cluster, select the policy name from the **Policy** drop-down.
    ...
    You can [create a Kubernetes policy](kubernetes-protection-policies.md) and assign the policy after connecting the cluster.
...
12. For users deploying Container Security protection the cluster for the first time:
    ...
    2.  To enable automated cluster registration, [create an API key](obtain-apikey-auto-cluster-reg.md) and enter `true` for `clusterRegistrationKey`, as shown in the example below.
