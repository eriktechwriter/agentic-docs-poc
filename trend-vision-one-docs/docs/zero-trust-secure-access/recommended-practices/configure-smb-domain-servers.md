...
1.  Configure Kerberos on the Domain Controller:
    ...
    1.  Within Private Access, [define an application](../secure-access-configuration/private-access-configuration/internal-application-configuration/add-an-internal-application-to-private-access) within the application for Kerberos on the domain controller using wild card domains and ports.

    2.  [Create a secure access rule](../secure-access-rules/creating-pac-rule) to provide access to the file servers for the appropriate groups users/groups.

2.  Configure the file server:

    1.  [Configure a different application](../secure-access-configuration/private-access-configuration/internal-application-configuration/add-an-internal-application-to-private-access) for the file server using wild card domains and ports.
