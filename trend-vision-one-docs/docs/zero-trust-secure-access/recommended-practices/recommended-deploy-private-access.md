...
First, you [create a connector group and deploy the connector](../getting-started-with-zero-trust-secure-access/deployment-guides/setting-up-zero-trust-secure-access-private-access/private-access-connector-deployment). The connector group serves to expose the VLANs, allowing external users to access the internal services hosted within them.
...
After deploying the connector, it is essential to [add the internal applications](../secure-access-configuration/private-access-configuration/internal-application-configuration/add-an-internal-application-to-private-access) in the Trend Vision One console to enable the connector to identify which applications it can access on the internal network.
...
To manage access permissions for the applications we just created, it is essential to [create private access rules](../secure-access-rules/creating-pac-rule).
