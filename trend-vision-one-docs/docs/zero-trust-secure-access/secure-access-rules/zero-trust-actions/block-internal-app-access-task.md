---
slug: trend-vision-one-block-internal-app-access-task
sidebar_position: 3
title: Block Internal App Access task
---

You can take remediation measures on user accounts that may pose a security risk to your network environment on the Trend Vision One console.

### Procedure {#procedure}

1.  Locate the user account that you want to take action on using the **Zero Trust Secure Access** or **Threat and Exposure Management** app.

    - **Zero Trust Secure Access**: Go to the **User Information** screen by clicking on a user anywhere within the **Zero Trust Secure Access** app.

    - **Threat and Exposure Management**: Go to the **User Profile** screen.

2.  Click the options icon (![](/images/options_icon=1b2aff71-0b33-4aa1-b8aa-6105438834e7.webp)) and **Block Internal App Access**.

    The **Block Internal App Access Task** screen appears.

3.  Click **Deploy Now** to deploy the Trend Micro private access service.

    :::note
    This step is required only when the private access service is not deployed to your corporate network. For more information, see [Private Access configuration](../../secure-access-configuration/private-access-configuration).

    The task stops proceeding. After you complete the private access service deployment, restart the task.
    :::

4.  (Optional) Specify a description for the task.

5.  Click **Create**.

    Trend Vision One creates the task and displays the current action status on the **Secure Access History** screen.

6.  Monitor the task status.

    1.  Click **View task status** in the success message that pops up in the lower right corner.

        The **Remediation Logs** tab on the **Secure Access History** screen appears.

        You can also go directly to **Secure Access History → Remediation Logs**, and locate the task using the search field.

    2.  View the task status under **Action status**.

        - *In progress...* (![](/images/inProgress=763d5319-3680-4b10-abca-a75782d8eb48.webp)): Trend Vision One sent the command to the enforcement point and is waiting for a response

        - *Successful* (![](/images/successful=d385863c-ae1f-4a76-8500-6d60d7a2de8f.webp)): The enforcement point successfully received and executed the command

        - *Unsuccessful* (![](/images/unsuccessful=e4c3ece6-3776-45dc-b99a-67195d7d21e0.webp)): An error or time-out occurred when attempting to send the command to the enforcement point

        :::note
        The task status indicates whether the enforcement point was able to successfully receive and execute the command. It may take a few minutes for the process to complete.
        :::

    After resolving the risk issues on the user account, you can allow the user account to access your internal apps using the options icon (![](/images/options_icon=1b2aff71-0b33-4aa1-b8aa-6105438834e7.webp)) in the **Zero Trust Secure Access** or **Threat and Exposure Management** app.

    For more information, see [Unblock Internal App Access task](./unblock-internal-app-access-task).
