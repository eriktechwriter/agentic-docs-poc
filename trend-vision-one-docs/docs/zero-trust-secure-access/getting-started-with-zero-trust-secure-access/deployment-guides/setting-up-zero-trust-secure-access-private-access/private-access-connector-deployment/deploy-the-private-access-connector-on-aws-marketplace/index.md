...
Private Access Connectors connect your internal applications with Zero Trust Secure Access Private Access, which allows you to control access to sensitive corporate resources. To ensure high availability (HA) and facilitate load-balancing on high traffic apps, install and group together at least 2 connectors in each environment. Before attempting to deploy the Private Access Connector, ensure that your environment meets the minimum [system requirements](../../../../preparing-to-deploy-private-access-internet-access-and-ai-service-access-services/system-requirements/pa-connector-system-reqs).
...
16. (Optional) Scale the VM instances either by [manually specifying a fixed Auto Scaling group size](./manual-scaling) or via [dynamic scaling policies](./automatic-scaling).
...
18. Use the CLI to configure other settings, if required.
    ...
    For more information on available commands, see [Private Access Connector CLI commands](../private-access-connector-cli).

    IMDSv2 Required option is supported in AWS connectors. For deployment details, see[Configure IMDSv2 in AWS deployments](./configure-imdsv2-in-aws-deployments) .
...
- [Manual Scaling](./manual-scaling "Manually scale your Private Access Connector VM instances to a fixed Auto Scaling group size.")
- [Automatic Scaling](./automatic-scaling "Scale your Private Access Connector VM instances based on metric thresholds by creating dynamic scaling policies.")
- [Configure IMDSv2 in AWS deployments](./configure-imdsv2-in-aws-deployments)
