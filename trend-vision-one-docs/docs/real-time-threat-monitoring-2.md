...
1.  [Activity Dashboard](activity-dashboard.md) - identify unusual user activities

2.  [Monitoring Dashboard](monitoring-dashboard.md) - get an in-depth record of all events in an AWS account
...
- 350+ rules running in real-time whenever resources are modified\_
  ...
  :::note
  The number of rules covered does not match the\_ [total number of rules](introduction-rules.md) that Cloud Risk Management supports. This is because RTPM is set up to cover the most used AWS services. New services or not-so-critical services are not integrated with RTPM
  :::
...
1.  Follow the instructions on [setting up Real-Time Posture Monitoring](realtime-threat-monitor.md)
