...
To enable user access control [response actions](../../../../introduction/trend-vision-one-proactive-security-starts-here/context-menu/response-actions-intro) on connected Active Directory accounts, you must [configure the Active Directory server connection](.) using a service account with sufficient permissions. You can grant the necessary permissions using one of the following three options.
