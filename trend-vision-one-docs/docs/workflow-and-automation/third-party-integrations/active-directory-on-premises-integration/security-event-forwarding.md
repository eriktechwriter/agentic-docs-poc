...
- Security Event Forwarding requires a [Service Gateway API key](../../service-gateway-management). To obtain the key, go to **Workflow and Automation → Service Gateway Management** and click **Manage API Key**.
...
4.  In the Trend Vision One console, verify that the agent is connected and perform additional integration steps if necessary.
    ...
    1.  Go to **Workflow and Automation → Third-Party Integrations → Active Directory (on-premises) → Security Event Forwarding**.
