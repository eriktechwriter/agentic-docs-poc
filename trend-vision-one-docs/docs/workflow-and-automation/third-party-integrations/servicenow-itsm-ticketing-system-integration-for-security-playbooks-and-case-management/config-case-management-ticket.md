...
sidebar_position: 2
title: Configure ServiceNow ITSM to enable Trend Vision One for ServiceNow ticketing system
---
...
2.  Create a non-interactive user restricted from directly signing in to the ServiceNow console.
    ...
    3.  Select an existing user or create a new user and then enable the **Web Service Access Only** check box.
        ...
        - x_tremi_trend_micr.security_layer_user
...
9.  For Automated Response Playbooks, if you enabled the **Open a Workbench case** option in the Target Settings, enable the ServiceNow integration settings in the Trend Vision One console:
    ...
    3.  Enable Case Management to send tickets to ServiceNow.
        ...
        After authenticating the connection, Case Management can send cases to ServiceNow for management. Changes made in ServiceNow sync with Trend Vision One.

    :::warning[Important]
    Cases created by Automated Response Playbooks use the **Trend Vision One Case Management** profile by default. This profile is automatically generated by the system but requires manual configuration. Make sure the profile is properly configured. For details on creating a ticket profile, see [Create a ticket profile for ServiceNow ITSM](../../case-management/trend-vision-one-cases/create-case-management-ticket-profiles/creating-ticket-profile).
    :::
