...
:::note
Taking certain response actions on critical endpoints could obstruct key operations or services. For critical devices, Trend Micro recommends [excluding the endpoints](../response-management-settings/response-exclude-endpoints) from certain response actions instead.

To allow network traffic on isolated critical endpoints, such as DNS servers, proxy servers, or VPN routers, you can [add an inbound or outbound network traffic exception](../response-management-settings/allow-traffic-isolated-endpoints) for the IP addresses of the endpoints.
:::
...
5.  Monitor the task status.
    ...
    For more information, see [Restore Connection task](./restore-connection-task).
