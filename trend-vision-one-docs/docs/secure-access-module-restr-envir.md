...
- Sign in to Trend Vision One. (excluding [NTLM single sign-on for Internet Access](ntlm-single-sign-internet-access.md))
...
2.  Go to **Agent Installer Proxy** and configure your proxy settings for **Sensor only** agents.
    ...
    For more information, see [Agent Installer Proxy Settings](agent-installer-proxy-settings.md).
...
4.  Go to **Runtime Proxy Settings** and configure your proxy settings in **Sensor Policy**.
    ...
    For more information, see [Runtime Proxy Settings](runtime-proxy-settings.md).
    ...
    For more information, see [Service Gateway services](service-gateway-services.md) .
    :::

    :::note
    To use a Service Gateway for traffic forwarding, refer to [Getting started with Service Gateway](getting-started-service-gateway.md) to deploy a Service Gateway with Forward Proxy Service enabled.
    :::
...
5.  To allow access to all services if you are using a Service Gateway:
    ...
    4.  If you select **Allow access to specified services**, add the [required FQDN or IP address](port-fqdn-ip-address-requirements.md) to **Allowed Service Setting**.

    For more information, see [Forward Proxy Service](forward-proxy-service.md).
...
7.  Switch to **Advanced mode** and delete all entries in the `SkipHost` list, as shown below.
    ...
    For more information, see [PAC file configuration](pac-file-configuration.md).
