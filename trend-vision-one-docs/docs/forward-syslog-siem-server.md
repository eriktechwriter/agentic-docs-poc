...
:::tip
Alternatively, if you want to publish events to Amazon SNS, see [Access events with Amazon SNS](set-up-amazon-sns.md).
:::
...
All routers, firewalls, and security groups must allow inbound traffic from Server & Workload Protection (and, for direct forwarding of security events, inbound traffic from agents) to your Syslog server. See also [Server & Workload Protection Port numbers](swp-port-numbers.md). Your Syslog server must be accessible via the Internet and its domain name must be globally DNS-resolvable.
...
3.  On the *General* tab, configure:
    ...
    - *Server Port:* Listening port number on the SIEM or Syslog server. For UDP, the IANA standard port number is 514. For TLS, it's usually port 6514. See also [Server & Workload Protection Port numbers](swp-port-numbers.md).
    ...
    - *Event Format:* Whether the log message's format is LEEF, CEF, or basic Syslog. See [Syslog message formats.](syslog-message-formats.md)
...
Like other policy settings, you can override event forwarding settings for specific policies or computers. See [Policies, inheritance, and overrides](policies-inheritance-overrides.md).
