...
16. (Optional) Scale the VM instances either by [manually specifying a fixed Auto Scaling group size](manual-scaling.md) or via [dynamic scaling policies](automatic-scaling.md).
...
18. Use the CLI to configure other settings, if required.
    ...
    For more information on available commands, see [Private Access Connector CLI commands](private-access-connector-cli.md).

    IMDSv2 Required option is supported in AWS connectors. For deployment details, see[Configure IMDSv2 in AWS deployments](config-imdsv2-aws-deployment.md) .
...
- [Configure IMDSv2 in AWS deployments](config-imdsv2-aws-deployment.md)
