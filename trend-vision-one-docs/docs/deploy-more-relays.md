...
[If you need to deploy more relays](how-relays-work.md), you must:
...
- *Geographic region and distance*: If you're deploying your own relays, each geographic region should have its own [relay group](how-relays-work.md) with at least 2 relays and agents should use relays in their same geographic region. Long distance and network latency can slow down update redistribution. Downloading from other geographic regions can also increase network bandwidth and/or cloud costs.
...
- *Use of Application Control shared rulesets through a proxy connection*: If you will use shared Application Control rulesets and [agents connect through a proxy](configure-proxies-server-workload.md), you might want to add more relays to handle large rulesets and improve performance. See [Deploy Application Control rulesets via relays](api-shared-global-rulesets.md) and [Server & Workload Protection Sizing](sizing-server-workload.md).
...
2.  Make sure you allow inbound and outbound communication to and from the relay on the appropriate port numbers. See [Server & Workload Protection port numbers](swp-port-numbers.md).

3.  Deploy an agent on the chosen computer. See [Get Deep Security Agent software](get-agent-software.md) and [Install the agent](install-agent.md).
...
You must indicate which relay group each agent should use. Either assign each agent to a relay group manually, or set up an [event-based task](autotasks-computer-changed-event.md) to assign new agents automatically.
...
2.  Under *Software Updates*, in *Alternate software update distribution server(s) to replace Deep Security Relays*, type:
    ...
    where `<IP>` is the private network IP address of the relay, and `<port>` is the [relay port number](swp-port-numbers.md)
