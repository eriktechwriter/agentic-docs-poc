...
- Before connecting an Amazon EKS Fargate pod, ensure that the container meets the necessary [system requirements for EKS Fargate](amazon-eks-fargate-sys-reqs.md) deployments.
:::
...
12. For users deploying Container Security protection the cluster for the first time:
    ...
    3.  To enable automated cluster registration, [create an API key](obtain-apikey-auto-cluster-reg.md) and enter `true` for `clusterRegistrationKey`, as shown in the example below.
