...
Time to live (TTL) configuration allows you to specify the length of time a [Check](model-check.md) will be displayed on the [All Checks Report](all-checks-report.md). The configuration is only available for certain [Rules](introduction-rules.md) that are specific to [Real Time Posture Monitoring](real-time-threat-monitoring.md).

For example:

1.  A user signs in without MFA. The rule [AWS IAM user has signed in without MFA](https://www.cloudconformity.com/knowledge-base=96b226c5-530f-49eb-b89e-74625f50f5f0/aws/RTPM/iam-user-sign-in-without-mfa.md) run against the resources associated with your cloud account to create an event on RTPM and a check on the [All Checks Report](all-checks-report.md)
...
4.  However, the entire event history will still show in the [Real Time Posture Monitoring](real-time-threat-monitoring.md) dashboard
