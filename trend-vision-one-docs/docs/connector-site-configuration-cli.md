...
You need a new registration token from the Trend Vision One console for an ESXi platform. The instructions below were taken from [Deploy the Private Access Connector on VMware ESXi](deploy-pac-vmware-esxi.md).
...
You now configure the connector. Note that the instructions below use Germany, "de", for the region. For Spain, replace "de" with "es". For more information on the commands, see [Private Access Connector CLI commands](private-access-connector-cli.md).
