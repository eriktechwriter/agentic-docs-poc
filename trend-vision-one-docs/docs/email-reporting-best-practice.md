...
1.  Set up reporting channels for your users.
    ...
    - Install the [Add-in for Outlook](add-for-outlook.md) in your users' Outlook client to enable one-click reporting of suspicious emails, or

    - Enable the [warning banner](correlated-intelligence.md) to display a cautionary message at the top of the email body flagged as anomalies by predefined correlation rules.

2.  Configure [email reporting](email-reporting.md) settings.
...
3.  Review [reported email details](user-reported-emails.md) and access the [analysis details screen](email-reporting-analysis-details.md) to view classification, suspicious indicators, email metadata.

4.  [Monitor the automated mitigation and prevention actions](email-reporting-analysis-details.md).

    - If [automated security actions](email-reporting.md) are enabled, the system automatically takes action on similar historical emails and apply prevention measures.

    - Use the progress bar to track real-time status updates.

5.  Apply security actions manually.

    If [manual security actions](email-reporting.md) are enabled, use the [analysis details screen](email-reporting-analysis-details.md) screen to take manual steps:
