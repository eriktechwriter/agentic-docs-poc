...
5.  (Optional) Grant required permissions on the IAM system that manages the user account.
    ...
    :::note
    This step is required only when the policy enforcement permission is not granted and the task fails due to insufficient permissions. For more information, see [Supported IAM systems and required permissions](iam-systems-required-permissions.md).
    :::
...
6.  (Optional) Assign the password administrator role to Trend Vision One.
    ...
    1.  Sign in to the [Microsoft Azure](https://portal.azure.com) portal and assign the password administrator role to Trend Vision One.
        ...
        For more information, see [Assigning the password administrator role](assign-password-administrator-role.md).
