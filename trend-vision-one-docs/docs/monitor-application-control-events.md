...
You can configure some of which Application Control event logs are recorded, and which are [forwarded to external SIEM systems or syslog servers](forward-syslog-siem-server.md).
...
3.  If you want to record event logs for that type of event, select *Record*.
    ...
    When those events occur, they appear on **Events → Reports → Events → System Events**. Logs are kept until they meet maximum log age criteria. For details, see [Event collection in Server & Workload Protection](event-logging.md).
...
5.  If you use an external SIEM, you may need to load the list of possible Application Control event logs, and indicate what action to take. For a list of Application Control events, see [Application Control events](application-control-events.md).
...
- *System event:* An audit event that provides a history of configuration changes or software updates. To see system events click **Events & Reports → Events → System Events**. For a list, see [System events](system-events.md).
...
To configure which Application Control events or severity levels cause an alert, go to the *Alerts* tab, click the *Configure Alerts* button, and then select an event and double-click *Properties*. For details, see [Configure alerts](configure-alerts.md).
