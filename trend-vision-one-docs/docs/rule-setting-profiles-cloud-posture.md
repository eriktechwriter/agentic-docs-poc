...
Profiles allow your organization to store and manage rule settings in reusable templates (e.g. rule settings for a specific environment type, security level, or application). You can apply a profile to one or more [Cloud Accounts](cloud-accounts-cloud-posture-setup.md) and also apply multiple profiles to an account to achieve the desired outcome for your rule settings.
...
3.  You can [configure every rule](rules-configuration.md) within a profile as per your requirements.
...
2.  Select account(s) from the list or search for it using [name, environment](cloud-account-general-settings.md), or [account tags](cloud-account-tags.md).
