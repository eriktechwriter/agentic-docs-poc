...
- User account: Your account must have API access and a role with permissions necessary to perform tasks.
  ...
  For more information on how to create a user account with API access, see [Configure accounts](configure-vision-accounts.md).

- Authentication token: The Trend Vision One console automatically generates a token for each account with API access.

  For more information on the authentication token, see [Obtaining API keys for third-party apps](api-keys-third-party-apps.md).

:::note
Trend Micro recommends using the latest versions of the APIs to stay current with updates to Trend Vision One and connected products. To get the latest API changes, click **Configure Change Log Notifications** to enable your [subscription](subscriptions-intro.md) in the **Notifications** app.
:::
