...
[Recommendation scans](recommendation-scans-intro.md) identify [Intrusion Prevention](intrusion-prevention.md), [Integrity Monitoring](integrity-monitoring.md), and [Log Inspection](log-inspection.md) rules that you should assign to or remove from a computer or policy. The application programming interface (API) provides access to recommendation scan results for these protection modules at the computer and policy levels via the following classes:
...
When running [enhanced recommendation scans](enhanced-reco-scan.md), the following API classes return empty values for `recommendedToAssignRuleIDs` and `recommendedToUnassignRuleIDs`:
...
The API provides access to [recommendation scan](recommendation-scans-intro.md) results for a computer for the Integrity Monitoring, Intrusion Prevention, and Log Inspection. Use a `ComputerIntrusionPreventionAssignmentsRecommendationsApi` object to obtain an `IntrusionPreventionAssignments` object for a computer. The `IntrusionPreventionAssignments` object contains and provides access to the recommendations for that computer:

- Recommended Intrusion Prevention rules to assign and unassign
- Scan status
- When the last scan occurred

After you obtain the rule recommendations, you can apply them to computer policies, as illustrated in the [Add intrusion prevention rules to computers' policies](patch-unprotected-computers.md) example.
...
Also see the [List Intrusion Prevention Rule IDs](https://cloudone.trendmicro.com/docs/workload-security/api-reference/tag/Computer-Intrusion-Prevention-Rule-Assignments-and-Recommendations) operation in the API Reference. For information about authenticating API calls, see [Authenticate with Server & Workload Protection](send-your-first-request-using-api.md).
