...
Trend Vision One™ – Cloud Risk Management provides you with the ability to suppress failed [Checks](model-check.md) - this may be useful for specific Rules that aren't relevant or are managed in a different way. Suppressed Checks will appear greyed out on the dashboard, and will display 'suppressed' counts on the [All Checks report](all-checks-report.md). Currently, we do not support batch suppression.
...
No, Suppress is at the Check level. You have the option to [disable rules](rules-configuration.md).
