...
2.  Create a working copy of Auto-remediation repository:
    ...
    :::note
    !!! note "" For self-healing capabilities, we recommend deploying auto-remediation in the same region as your AWS account. Detection of Check failures is independent of the region of deployment as it is performed by [RTM](real-time-threat-monitoring.md) and [Cloud Risk Management Bot](cloudIdentityBot.md).
    ...
    10\. Integrate [Amazon SNS Communication](amazon-sns-communication.md) channel on the Cloud Risk Management platform and select the notifications you wish to enable:
...
1.  Verify that the [Communication trigger](communication-triggers.md) that you selected while configuring an [Amazon SNS channel](amazon-sns-communication.md) is:
...
4.  Click *Create Security Group*
    ...
    - If you have enabled [Real-Time Threat Monitoring](real-time-threat-monitoring.md) you will instantly see an entry in the [Activity Dashboard](activity-dashboard.md) and [Monitoring Dashboard](monitoring-dashboard.md) when Auto-remediation kicks in.
    - If you have only enabled [Automatic notifications](toggle-automatic-notifications.md), the Check failure will be automatically resolved.

## Resolution using Manual notifications {#resolution-using-manual-notifications}

If you have only enabled [Manual notifications](toggle-manual-notifications.md), follow the steps below to resolve the failure:

### Procedure {#procedure-4}

1.  Go to [All Checks report](all-checks-report.md) and filter rules by:
