...
2.  In the **Quarantined Message Permissions** section, specify the permissions that end users will have on the email messages quarantined for a specific reason.
    ...
    For more information about the quarantine reason, see [Querying the quarantine](querying-quarantine.md).
    ...
    :::note
    The "Deliver", "Delete", and "Block Sender" actions are available for messages quarantined for all reasons listed. The "Approve Sender" action, however, is available only for messages quarantined for the reasons under the Spam Filtering category. For more information, see [Configuring approved and blocked sender lists](config-approved-blocked-sender.md).
    :::
