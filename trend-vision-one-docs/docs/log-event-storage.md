...
See [How long are events stored?](event-logging.md) for details on the storage period for system and security events.

Steps to consider to control storage:

### Procedure {#procedure}

1.  Forward security events to external storage. See [Forward events to an external Syslog or SIEM server](forward-syslog-siem-server.md).

2.  Set thresholds in the log inspection module for event storage or event forwarding. *Severity clipping* allows you to send events to a Syslog server (if enabled) or to store events based on the severity level of the log inspection rule. See [Thresholds for Event Storage or Event Forwarding](set-up-log-inspection.md).
