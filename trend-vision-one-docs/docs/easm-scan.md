...
- External attack surface scans are only available when using Trend Micro solutions for internet-facing asset discovery. Scans are not supported if you are only using a third-party product for asset discovery. To discover assets on your network, consider creating and running a [discovery scan](discover-scan.md).

- Service Gateways used for scanning must be deployed to a public cloud platform such as AWS or Microsoft Azure and running to successfully scan external assets. Service Gateways not deployed to a cloud platform may be unable to reach internet-facing assets due to restrictions on your network. To learn more, see [Deploy a Service Gateway virtual appliance with AWS](deploy-service-gateway-aws.md) and [Deploy a Service Gateway virtual appliance with Microsoft Azure](service-gateway-azure.md).

- Service Gateways used for scanning must have the Network Vulnerability Scanner service version 1.1.0 or later installed. If necessary, you can update the Network Vulnerability Scanner service in [Service Gateway Management](managing-services-service-gateway.md).
...
2.  Create a new external attack surface scan.
    ...
    4.  Click **Select assets** to choose up to 100 public IP addresses, domains, and subdomains to include in the scan. Only assets that appear on the **Internet-facing assets** list **Attack Surface Discovery** are available for scanning. To scan an asset not on the list, you must add the asset first.
        ...
        :::note
        You must have the **View** permission for Attack Surface Discovery to select target assets.
        :::
...
:::tip
You can also manually scan for exposures in internet-facing assets from **Attack Surface Discovery**. For more information, see [Internet-facing asset exposure scans](internet-facing-asset-scan.md).
:::
