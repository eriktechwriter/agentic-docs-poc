...
:::warning[Important]
If you deployed your Server & Workload Protection agents using the agent installer package in Endpoint Inventory, you must use the [Agent Installer Proxy](agent-installer-proxy-settings.md) and [Runtime Proxy Settings](runtime-proxy-settings.md) in Global Settings to configure your proxy connections.

Agents migrated from updating your Trend Cloud One Workload Security to Server & Workload Protection automatically adopt the [Agent Installer Proxy](agent-installer-proxy-settings.md) and [Runtime Proxy Settings](runtime-proxy-settings.md) in Global Settings.
...
:::note
The primary component update source for the new [improved relay](how-relays-work.md) is the Server & Workload Protection relay. Deep Security Agent 20.0.0-3964+ can connect to Server & Workload Protection relays via this proxy setting.
:::

You can connect your agents to your 'primary component update source' via a proxy. By default, the [primary component update source](deploy-more-relays.md) is the Trend Micro Update Server (also called ActiveUpdate).

:::note
The agents and appliances will only use the proxy if their assigned relay is not available, and they've been [granted explicit permission to access the primary update source](deploy-more-relays.md).
:::
...
- For details on `dsa_control` and its `-u` and `-x` options, see [dsa_control](command-line-basics.md).
...
- For details on dsa_control and its --pacproxy and --pacproxyunpw options, see [dsa_control](command-line-basics.md).
...
7.  Send the new policy to your agents. See [Send policy changes manually](create-policies.md).
...
Redeploy agents using new deployment scripts that no longer contain proxy settings. For details, see [Use deployment scripts to add and protect computers](deployment-scripts-add-computers.md).
...
For details on `dsa_control` and its `-x`, `-y`, and `--pacproxy` options, see [dsa_control](command-line-basics.md).
