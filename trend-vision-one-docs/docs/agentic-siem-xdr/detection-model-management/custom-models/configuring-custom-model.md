...
4.  Specify the filter settings:
    ...
    - **Filter name**: the filter the custom model uses
      ...
      :::note
      [Create custom filter](../custom-filters/creating-custom-filter) opens **Custom Filter Settings** in a new browser tab. After creating the new custom filter, you can select the filter from **Filter name** in **Custom Models**.
      :::
...
7.  Specify the event grouping.
    ...
    - **Container**: Trigger an event when the threshold is met and associated with a single container.
