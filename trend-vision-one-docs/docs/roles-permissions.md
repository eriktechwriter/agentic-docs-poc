...
An Administrator user can [assign roles](add-user.md) to other users.

## Administrator {#administrator}

The Administrator role has access to all of the Cloud Risk Management features including [addition of new users](add-user.md) and [managing subscriptions](manage-subscriptions.md). This role is at a system level rather than at an account level, implying that an Administrator role can see all accounts within an organization and has complete access to all API endpoints.
