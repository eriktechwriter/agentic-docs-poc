...
In most cases, use of the [standard deployment scripts](deployment-scripts-add-computers.md) (which, by the way, also use this same URL format described in this section to download the agent software) is the quickest way to get started and will meet the majority of your deployment requirements.
...
The [agent version control](configure-agent-version-control.md) feature provides the ability to control what agents are returned when any URL request is made to Server & Workload Protection to download the agent.
