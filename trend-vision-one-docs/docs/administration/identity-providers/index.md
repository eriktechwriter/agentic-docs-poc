...
3.  Go to your IdP to configure the settings and download the IdP metadata XML file.
    ...
    - [Configure Active Directory Federation Services](./configuring-adfs)

    - [Configure Google Cloud Identity](./config-google-cloud-saml)

    - [Configure Microsoft Entra ID](./configuring-saml-ad)

    - [Configure Okta](./configure-okta)

    - [Configure OneLogin](./configuring-onelogin)
...
7.  (Optional) Configure the IdP to support adding [IdP-Only SAML Group Account](../user-accounts/configure-accounts/adding-idp-only-saml-group-account) users by specifying attributes provided in the IdP system.
...
10. Click **Close** to return to the **Identity Providers** screen, or click **Add SAML Users** to add SAML user accounts in **User Accounts**.
    ...
    For more information, see [Configure accounts](../user-accounts/configure-accounts).

**Related information**

- [Configure Active Directory Federation Services](./configuring-adfs "Configure Active Directory Federation Services (AD FS) as a SAML (2.0) identity provider for Trend Vision One.")
- [Configure Google Cloud Identity](./config-google-cloud-saml "Configure Google Cloud Identity to serve as a SAML (2.0) identity provider for use by Trend Vision One.")
- [Configure Microsoft Entra ID](./configuring-saml-ad "Configure Microsoft Entra ID as a SAML (2.0) identity provider for Trend Vision One to use.")
- [Configure Okta](./configure-okta "Configure Okta as a SAML (2.0) identity provider for Trend Vision One to use.")
- [Configure OneLogin](./configuring-onelogin "Configure OneLogin as a SAML (2.0) identity provider for Trend Vision One.")
