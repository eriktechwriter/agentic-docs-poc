...
Control or limit which screens and apps each account can access by assigning [user roles](../user-roles) to your accounts.
...
:::note
As of October 27, 2025, SIEM & SOAR user roles cannot be assigned to user accounts. User accounts granted a SIEM or SOAR role before that date retain their role, but it is recommended that they be assigned a different role since SIEM and SOAR roles are intended for use with API keys and are not suitable for user accounts.
:::
...
- [Configure accounts](./configure-accounts "Create and manage Trend Vision One user accounts to allow authorized users to access the console.")
- [Enable and configure multi-factor authentication](./enable-confige-mfa "Enforce multi-factor authentication (MFA) to enhance account security and prevent attackers from exploiting critical actions for malicious activities.")
