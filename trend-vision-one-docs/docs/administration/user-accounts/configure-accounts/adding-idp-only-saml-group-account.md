...
- Provided that an IdP-Only SAML Group Account user is not a user of a SAML Account or SAML Group Account, the user can switch among multiple accounts with different user roles and asset visibility scopes. For more information, see [User account switch](../../../introduction/trend-vision-one-proactive-security-starts-here/user-account-switch).
:::
...
5.  Select a **Role**.
    ...
    For more information, see [User Roles](../../user-roles).
...
7.  Select an IdP from which to select groups that can access the Trend Vision One console.
    ...
    You can find all the IdPs that have been added in [Identity Providers](../../identity-providers) in the drop-down list, but only the IdPs that are configured to support IdP-Only SAML Group Account are available to select.
