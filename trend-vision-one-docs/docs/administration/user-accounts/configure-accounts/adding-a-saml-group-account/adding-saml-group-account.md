...
This optimization requires access to the Microsoft Entra ID tenants where the groups reside. For SAML groups added before June 30, 2025 with no tenant access granted, you need to manually grant the access in **Third-Party Integrations** for the groups to support SSO access and stay current. For details, see [Configure Microsoft Entra ID integration](../../../../workflow-and-automation/third-party-integrations/microsoft-entra-id-integration/configuring-integration). Make sure you grant at least one of the permission sets whose associated apps include **User Accounts**.
:::
...
1.  Make sure that Microsoft Entra ID is configured in **Administration → Identity Providers** to set up SSO authentication between Microsoft Entra ID and Trend Vision One.
    ...
    For details, see [Configure Microsoft Entra ID](../../../identity-providers/configuring-saml-ad).

2.  Make sure that you have granted required access to Microsoft Entra ID in **Workflow and Automation → Third-Party Integrations** to synchronize group data from the identity provider.

    For details, see [Configure Microsoft Entra ID integration](../../../../workflow-and-automation/third-party-integrations/microsoft-entra-id-integration/configuring-integration).
...
7.  Select **Tenant**.
    ...
    If you cannot find your target tenant, check whether you have granted the proper access to the tenant in **Third-Party Integrations**.
...
10. Select a Trend Vision One role to assign to the users in the group.
    ...
    To create a custom user role, click **Create a custom role in User Roles**. For more information, see [User Roles](../../../user-roles).
...
13. Make sure that group users verify their email addresses.
    ...
    - If one or more of your domains have been verified using [Domain Verification](../../../domain-verification), all SAML users or user group members under a verified domain can be added directly without the need to verify email addresses.
    :::
