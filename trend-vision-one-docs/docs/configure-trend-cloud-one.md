...
Trend Micro recommends transferring all product and base agent information before contacting support to ensure that XDR Endpoint Sensor agents associate with the right endpoints.

After connecting Trend Cloud One, Endpoint & Workload Security is no longer available in the Trend Cloud One environment, and all endpoint management transfers to Trend Vision One in Server & Workload Protection and Endpoint Inventory.
:::
